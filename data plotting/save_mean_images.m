function meanImgs = save_mean_images( ...
    ...
    %   meanImgs{m}{p}
    %
    %   m = index du recording
    %   p = index du plan
    %
    % Si l'image TIF existe déjà, elle est chargée sans recalcul.

    numRecordings = numel(current_gcamp_folders);

    meanImgs = cell(1, numRecordings);
    ...
    for m = 1:numRecordings

        fall_paths = current_gcamp_folders{m};
        ...
        % =============================================================
        % Age et date du recording courant
        % =============================================================

        age_name = get_age_name( ...
            current_ages, ...
            m);

        date_name = get_date_name( ...
            current_dates, ...
            m);
        ...
        if nPlanes == 0
            ...
            fprintf( ...
                'Status  : no plane available for this channel.\n');
        ...
        for p = 1:nPlanes
            ...
            tif_filename = fullfile( ...
                output_folder_plane, ...
                sprintf( ...
                    'Mean_image_%s_of_%s_%s_%s_plane%d.tif', ...
                    label_clean, ...
                    animal_clean, ...
                    date_clean, ...
                    age_clean, ...
            ...
            if isfile(tif_filename)
                ...
                try
                    ...
                    meanImgs{m}{p} = ...
                        double(imread(tif_filename));

                    valid_mask(p) = true;

                    fprintf( ...
                        '    Status: existing mean image loaded.\n');
                    ...
                    fprintf( ...
                        ['    Status: existing file unreadable; ' ...
                         'recalculation attempted.\n']);
            ...
            if isempty(fall_path)
                ...
                fprintf( ...
                    ['    Status: skipped because Suite2p ' ...
                     'path is empty.\n']);
            ...
            if exist(ops_npy, 'file') == 2
                ...
                try
                    ...
                    fprintf( ...
                        '    Status: ops.npy could not be read.\n');
            ...
            elseif exist(ops_mat, 'file') == 2
                ...
                try
                    ...
                    fprintf( ...
                        '    Status: ops.mat could not be read.\n');
                ...
                fprintf( ...
                    '    Status: no Suite2p ops file found.\n');
            ...
            if isempty(meanImg)
                ...
                fprintf( ...
                    '    Status: Suite2p mean image is empty.\n');
            ...
            try
                ...
                fprintf( ...
                    '    Status: mean image created and saved.\n');
        ...
        if numValidPlanes > 1
            ...
            root_folder_m = get_recording_output_root( ...
                gcamp_output_folders, ...
                m);
            ...
            if isempty(root_folder_m)
                ...
                warning( ...
                    ...
                    ['Recording %d/%d | missing root output ' ...
                     'folder for Z projection.'], ...
                    m, ...
                    numRecordings);

                fprintf( ...
                    ['    Status: Z projection skipped; ' ...
            ...
            if isfile(tif_global)
                ...
                fprintf( ...
                    '    Status: existing Z projection retained.\n');
            ...
            try
                ...
                warning( ...
                    ...
                    ['Recording %d/%d | unable to generate ' ...
                     'Z projection: %s'], ...
                    m, ...
                    numRecordings, ...
                    ME.message);
        ...
        elseif numValidPlanes == 1
            ...
            fprintf( ...
                ['    Status: not required; recording contains ' ...
                 'one valid plane.\n']);

        else

            fprintf( ...
                ['    Status: not generated; no valid mean image ' ...
                 'available.\n']);
...
function age_name = get_age_name(current_ages, m)
...
% =====================================================================
function date_name = get_date_name(current_dates, m)
    ...
    date_name = char(string(current_date));
...
% =====================================================================
function root_folder_m = get_recording_output_root( ...
    gcamp_output_folders, ...
    m)
