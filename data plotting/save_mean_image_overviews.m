function save_mean_image_overviews( ...
    current_line, ...
    current_animal, ...
    current_dates, ...
    current_ages, ...
    current_automatic_selection, ...
    current_suite2p_group, ...
    gcamp_root_folders, ...
    output_folders, ...
    meanImgs_gcamp, ...
    meanImgs_electroporated, ...
    alignedImgs_electroporated, ...
    metadata_gcamp, ...
    metadata_electroporated)

%SAVE_MEAN_IMAGE_OVERVIEWS
%
% Crée une figure par recording.
%
% Chaque ligne correspond à un plan.
%
% Le canal electroporated est déterminé automatiquement à partir de
% current_suite2p_group :
%
%   colonne 2 présente -> Electroporated affiché en rouge
%   sinon colonne 3     -> Electroporated affiché en bleu
%
% Si un canal electroporated existe :
%
%   Colonne 1 : GCaMP en vert
%   Colonne 2 : Electroporated en rouge ou bleu
%   Colonne 3 : Overlay
%
% Les coordonnées X / Y / Z sont extraites séparément depuis :
%
%   metadata_gcamp
%   metadata_electroporated
%
% Sauvegarde :
%
%   - toujours dans gcamp_root_folders{m}
%
%   - dans output_folders{m} si current_automatic_selection ~= 0
%
% IMPORTANT :
%
% output_folders{m} contient déjà le dossier final du recording :
%
%   ...\Development\line\animal\date
%
% ou
%
%   ...\Adult\line\animal\date
%
% Il ne faut donc PAS rajouter line / animal / date dans cette fonction.
%
% Aucun fichier existant n'est écrasé.
% line / animal / date sont utilisés tels quels, sans nettoyage.
% Seul l'âge numérique est extrait depuis current_ages.


    if nargin < 13

        error( ...
            'save_mean_image_overviews:MissingInputs', ...
            'Thirteen input arguments are required.');
    end


    if isempty(meanImgs_gcamp) && ...
            isempty(alignedImgs_electroporated)

        fprintf('\n');
        fprintf('No mean image available for overview figures.\n');

        return;
    end


    % =============================================================
    % Identité
    % =============================================================

    line_name = ...
        current_line;

    animal_name = ...
        current_animal;


    % =============================================================
    % Couleur du canal electroporated
    %
    % Colonne 2 -> Red
    % Colonne 3 -> Blue
    % =============================================================

    electroporated_color_channel = [];
    electroporated_color_name = '';


    for c = [2 3]

        if c > size(current_suite2p_group, 2)
            continue;
        end


        current_column = ...
            current_suite2p_group(:, c);


        has_electroporated_data = ...
            any( ...
                cellfun( ...
                    @(x) ...
                        ~isempty(x) && ...
                        (~ischar(x) || ...
                         ~isempty(strtrim(x))), ...
                    current_column));


        if ~has_electroporated_data
            continue;
        end


        if c == 2

            electroporated_color_channel = ...
                1;

            electroporated_color_name = ...
                'Red';

        else

            electroporated_color_channel = ...
                3;

            electroporated_color_name = ...
                'Blue';
        end


        break;
    end


    % =============================================================
    % Nombre de recordings
    % =============================================================

    num_recordings = ...
        max( ...
            numel(meanImgs_gcamp), ...
            numel(alignedImgs_electroporated));


    fprintf('\n');
    fprintf('============================================================\n');
    fprintf('MEAN IMAGE OVERVIEW FIGURES\n');
    fprintf('Line      : %s\n', line_name);
    fprintf('Animal    : %s\n', animal_name);
    fprintf('Recordings: %d\n', num_recordings);


    if isempty(electroporated_color_name)

        fprintf( ...
            'Electroporated channel: unavailable\n');

    else

        fprintf( ...
            'Electroporated channel: %s\n', ...
            electroporated_color_name);
    end


    fprintf('============================================================\n');


    % =============================================================
    % Recordings
    % =============================================================

    for m = 1:num_recordings

        % =========================================================
        % Age
        % =========================================================
        
        age_name = [];
        
        if ~isempty(current_ages) && ...
                m <= numel(current_ages)
        
            age_name = ...
                current_ages{m};
        end
        
        
        age_number = ...
            extract_age_number_mean_overview( ...
                age_name);

        % =========================================================
        % Date
        % =========================================================
        
        date_name = [];
        
        if ~isempty(current_dates) && ...
                m <= numel(current_dates)
        
            date_name = ...
                current_dates{m};
        end

        % =========================================================
        % Dossier summary du recording
        %
        % output_folders{m} contient déjà :
        %
        % ...\Development\line\animal\date
        %
        % ou
        %
        % ...\Adult\line\animal\date
        % =========================================================

        current_output_folder = ...
            get_root_folder_mean_overview( ...
                output_folders, ...
                m);


        % =========================================================
        % Images du recording
        % =========================================================

        gcamp_images_m = ...
            get_recording_images_mean_overview( ...
                meanImgs_gcamp, ...
                m);


        % Utiliser l'image alignée si disponible,
        % sinon utiliser directement la mean image electroporated

        electroporated_images_m = ...
            get_recording_images_mean_overview( ...
                alignedImgs_electroporated, ...
                m);


        if isempty(electroporated_images_m)

            electroporated_images_m = ...
                get_recording_images_mean_overview( ...
                    meanImgs_electroporated, ...
                    m);
        end


        num_planes = ...
            max( ...
                numel(gcamp_images_m), ...
                numel(electroporated_images_m));


        % =========================================================
        % Coordonnées GCaMP
        % =========================================================

        gcamp_position_x = ...
            get_metadata_recording_value_mean_overview( ...
                metadata_gcamp, ...
                'PositionX_um', ...
                m);


        gcamp_position_y = ...
            get_metadata_recording_value_mean_overview( ...
                metadata_gcamp, ...
                'PositionY_um', ...
                m);


        gcamp_position_z = ...
            get_metadata_recording_value_mean_overview( ...
                metadata_gcamp, ...
                'PositionZ', ...
                m);


        % =========================================================
        % Coordonnées Electroporated
        % =========================================================

        electroporated_position_x = ...
            get_metadata_recording_value_mean_overview( ...
                metadata_electroporated, ...
                'PositionX_um', ...
                m);


        electroporated_position_y = ...
            get_metadata_recording_value_mean_overview( ...
                metadata_electroporated, ...
                'PositionY_um', ...
                m);


        electroporated_position_z = ...
            get_metadata_recording_value_mean_overview( ...
                metadata_electroporated, ...
                'PositionZ', ...
                m);


        fprintf('\n');
        fprintf('------------------------------------------------------------\n');
        fprintf('Recording %d/%d\n', m, num_recordings);
        fprintf('Date      : %s\n', date_name);
        fprintf('Age       : %s\n', age_name);
        fprintf('Planes    : %d\n', num_planes);
        fprintf('------------------------------------------------------------\n');


        if num_planes == 0

            fprintf( ...
                'Status: no mean image available.\n');

            continue;
        end


        % =========================================================
        % Dossier local du recording
        % =========================================================

        output_root_folder = ...
            get_root_folder_mean_overview( ...
                gcamp_root_folders, ...
                m);


        local_png_path = ...
            '';


        local_destination_available = ...
            false;


        if isempty(output_root_folder)

            warning( ...
                'save_mean_image_overviews:MissingOutputFolder', ...
                ['Recording %d: gcamp_root_folders does not ' ...
                 'contain a valid output folder.'], ...
                m);

        else

            if ~isfolder(output_root_folder)

                try

                    mkdir( ...
                        output_root_folder);

                catch ME

                    warning( ...
                        'save_mean_image_overviews:FolderCreationFailed', ...
                        ['Unable to create output folder for ' ...
                         'recording %d: %s'], ...
                        m, ...
                        ME.message);
                end
            end


            if isfolder(output_root_folder)

                local_png_path = ...
                    fullfile( ...
                        output_root_folder, ...
                        sprintf( ...
                            'Mean_images_%s_%s_P%g.png', ...
                            animal_name, ...
                            date_name, ...
                            age_number));


                local_destination_available = ...
                    true;
            end
        end


        % =========================================================
        % Destination summary
        %
        % IMPORTANT :
        % current_output_folder est déjà le dossier final.
        % =========================================================

        summary_png_path = ...
            '';


        summary_destination_available = ...
            false;


        if current_automatic_selection && ...
                ~isempty(current_output_folder)


            if exist( ...
                    current_output_folder, ...
                    'dir') ~= 7

                mkdir( ...
                    current_output_folder);
            end


            summary_base_name = ...
                sprintf( ...
                    '%s_%s_%s_P%g_mean_images_overview', ...
                    line_name, ...
                    animal_name, ...
                    date_name, ...
                    age_number);


            summary_png_path = ...
                fullfile( ...
                    current_output_folder, ...
                    [summary_base_name '.png']);


            summary_destination_available = ...
                true;
        end


        % =========================================================
        % Vérification indépendante des destinations
        % =========================================================

        local_exists = ...
            local_destination_available && ...
            isfile(local_png_path);


        summary_exists = ...
            summary_destination_available && ...
            isfile(summary_png_path);


        local_needs_save = ...
            local_destination_available && ...
            ~local_exists;


        summary_needs_save = ...
            summary_destination_available && ...
            ~summary_exists;


        if local_exists

            fprintf( ...
                'Local overview already exists:\n%s\n', ...
                local_png_path);
        end


        if summary_exists

            fprintf( ...
                'Summary overview already exists:\n%s\n', ...
                summary_png_path);
        end


        if ~local_destination_available && ...
                ~summary_destination_available

            warning( ...
                'save_mean_image_overviews:NoValidDestination', ...
                ['Recording %d: no valid output destination ' ...
                 'is available.'], ...
                m);

            continue;
        end


        if ~local_needs_save && ...
                ~summary_needs_save

            fprintf( ...
                ['Status: all available overview files already ' ...
                 'exist, figure skipped.\n']);

            continue;
        end


        % =========================================================
        % Charger les images par plan
        % =========================================================

        gcamp_planes = ...
            cell( ...
                num_planes, ...
                1);


        electroporated_planes = ...
            cell( ...
                num_planes, ...
                1);


        has_gcamp_by_plane = ...
            false( ...
                num_planes, ...
                1);


        has_electroporated_by_plane = ...
            false( ...
                num_planes, ...
                1);


        for p = 1:num_planes

            gcamp_planes{p} = ...
                get_plane_image_mean_overview( ...
                    gcamp_images_m, ...
                    p);


            electroporated_planes{p} = ...
                get_plane_image_mean_overview( ...
                    electroporated_images_m, ...
                    p);


            has_gcamp_by_plane(p) = ...
                ~isempty( ...
                    gcamp_planes{p});


            has_electroporated_by_plane(p) = ...
                ~isempty( ...
                    electroporated_planes{p});
        end


        valid_planes = ...
            has_gcamp_by_plane | ...
            has_electroporated_by_plane;


        if ~any(valid_planes)

            fprintf( ...
                'Status: all planes are empty.\n');

            continue;
        end


        has_any_electroporated = ...
            any(has_electroporated_by_plane) && ...
            ~isempty(electroporated_color_channel);


        if has_any_electroporated

            num_columns = ...
                3;

        else

            num_columns = ...
                1;
        end


        valid_plane_indices = ...
            find( ...
                valid_planes);


        num_valid_planes = ...
            numel( ...
                valid_plane_indices);


        % =========================================================
        % Dimensions figure
        % =========================================================

        tile_width = ...
            480;


        tile_height = ...
            440;


        figure_width = ...
            max( ...
                600, ...
                num_columns * tile_width);


        figure_height = ...
            max( ...
                500, ...
                num_valid_planes * tile_height + 120);


        fig = [];


        try

            fig = ...
                figure( ...
                    'Visible', 'off', ...
                    'Color', 'w', ...
                    'Units', 'pixels', ...
                    'Position', [ ...
                        100, ...
                        100, ...
                        figure_width, ...
                        figure_height]);


            layout = ...
                tiledlayout( ...
                    fig, ...
                    num_valid_planes, ...
                    num_columns, ...
                    'TileSpacing', 'compact', ...
                    'Padding', 'compact');


            % =====================================================
            % Une ligne par plan
            % =====================================================

            for row_index = 1:num_valid_planes

                p = ...
                    valid_plane_indices( ...
                        row_index);


                mean_gcamp = ...
                    gcamp_planes{p};


                mean_electroporated = ...
                    electroporated_planes{p};


                has_gcamp = ...
                    has_gcamp_by_plane(p);


                has_electroporated = ...
                    has_electroporated_by_plane(p);


                % =================================================
                % Coordonnées du plan
                % =================================================

                gcamp_coordinate_text = ...
                    build_plane_coordinate_text_mean_overview( ...
                        gcamp_position_x, ...
                        gcamp_position_y, ...
                        gcamp_position_z, ...
                        p);


                electroporated_coordinate_text = ...
                    build_plane_coordinate_text_mean_overview( ...
                        electroporated_position_x, ...
                        electroporated_position_y, ...
                        electroporated_position_z, ...
                        p);


                % =================================================
                % GCaMP + Electroporated + Overlay
                % =================================================

                if has_any_electroporated

                    green_rgb = [];
                    electroporated_rgb = [];
                    overlay_rgb = [];


                    if has_gcamp

                        green_rgb = ...
                            create_single_channel_rgb( ...
                                mean_gcamp, ...
                                2);
                    end


                    if has_electroporated

                        electroporated_rgb = ...
                            create_single_channel_rgb( ...
                                mean_electroporated, ...
                                electroporated_color_channel);
                    end


                    if has_gcamp && ...
                            has_electroporated

                        [ ...
                            mean_gcamp_overlay, ...
                            mean_electroporated_overlay ...
                        ] = ...
                            match_mean_image_sizes( ...
                                mean_gcamp, ...
                                mean_electroporated);


                        overlay_rgb = ...
                            create_mean_image_overlay( ...
                                mean_gcamp_overlay, ...
                                mean_electroporated_overlay, ...
                                electroporated_color_channel);
                    end


                    % ---------------------------------------------
                    % GCaMP
                    % ---------------------------------------------

                    ax_gcamp = ...
                        nexttile( ...
                            layout);


                    if has_gcamp

                        image( ...
                            ax_gcamp, ...
                            green_rgb);

                    else

                        show_empty_mean_image_panel( ...
                            ax_gcamp, ...
                            'GCaMP unavailable');
                    end


                    axis( ...
                        ax_gcamp, ...
                        'image');


                    axis( ...
                        ax_gcamp, ...
                        'off');


                    title( ...
                        ax_gcamp, ...
                        sprintf( ...
                            'Plane %d - GCaMP\n%s', ...
                            p - 1, ...
                            gcamp_coordinate_text), ...
                        'Interpreter', 'none', ...
                        'FontSize', 12, ...
                        'FontWeight', 'bold');


                    % ---------------------------------------------
                    % Electroporated
                    % ---------------------------------------------

                    ax_electroporated = ...
                        nexttile( ...
                            layout);


                    if has_electroporated

                        image( ...
                            ax_electroporated, ...
                            electroporated_rgb);

                    else

                        show_empty_mean_image_panel( ...
                            ax_electroporated, ...
                            'Electroporated unavailable');
                    end


                    axis( ...
                        ax_electroporated, ...
                        'image');


                    axis( ...
                        ax_electroporated, ...
                        'off');


                    title( ...
                        ax_electroporated, ...
                        sprintf( ...
                            'Plane %d - Electroporated (%s)\n%s', ...
                            p - 1, ...
                            electroporated_color_name, ...
                            electroporated_coordinate_text), ...
                        'Interpreter', 'none', ...
                        'FontSize', 12, ...
                        'FontWeight', 'bold');


                    % ---------------------------------------------
                    % Overlay
                    % ---------------------------------------------

                    ax_overlay = ...
                        nexttile( ...
                            layout);


                    if has_gcamp && ...
                            has_electroporated

                        image( ...
                            ax_overlay, ...
                            overlay_rgb);

                    else

                        show_empty_mean_image_panel( ...
                            ax_overlay, ...
                            'Overlay unavailable');
                    end


                    axis( ...
                        ax_overlay, ...
                        'image');


                    axis( ...
                        ax_overlay, ...
                        'off');


                    title( ...
                        ax_overlay, ...
                        sprintf( ...
                            'Plane %d - Overlay', ...
                            p - 1), ...
                        'Interpreter', 'none', ...
                        'FontSize', 12, ...
                        'FontWeight', 'bold');


                % =================================================
                % GCaMP seul
                % =================================================

                else

                    ax_gcamp = ...
                        nexttile( ...
                            layout);


                    if has_gcamp

                        green_rgb = ...
                            create_single_channel_rgb( ...
                                mean_gcamp, ...
                                2);


                        image( ...
                            ax_gcamp, ...
                            green_rgb);

                    else

                        show_empty_mean_image_panel( ...
                            ax_gcamp, ...
                            'GCaMP unavailable');
                    end


                    axis( ...
                        ax_gcamp, ...
                        'image');


                    axis( ...
                        ax_gcamp, ...
                        'off');


                    title( ...
                        ax_gcamp, ...
                        sprintf( ...
                            'Plane %d - GCaMP\n%s', ...
                            p - 1, ...
                            gcamp_coordinate_text), ...
                        'Interpreter', 'none', ...
                        'FontSize', 12, ...
                        'FontWeight', 'bold');
                end
            end


            % =====================================================
            % Titre général
            % =====================================================

            sgtitle( ...
                layout, ...
                sprintf( ...
                    '%s | %s | %s | %s', ...
                    line_name, ...
                    animal_name, ...
                    date_name, ...
                    age_name), ...
                'Interpreter', 'none', ...
                'FontSize', 16, ...
                'FontWeight', 'bold');


            % =====================================================
            % Sauvegarde locale
            % =====================================================

            if local_needs_save

                save_mean_overview_figure( ...
                    fig, ...
                    local_png_path);


                fprintf( ...
                    'Local overview saved:\n%s\n', ...
                    local_png_path);
            end


            % =====================================================
            % Sauvegarde summary
            % =====================================================

            if summary_needs_save

                same_destination = ...
                    local_destination_available && ...
                    strcmpi( ...
                        char(local_png_path), ...
                        char(summary_png_path));


                if same_destination

                    if ~local_needs_save

                        fprintf( ...
                            ['Summary destination is identical to ' ...
                             'the existing local destination.\n']);
                    end

                else

                    save_mean_overview_figure( ...
                        fig, ...
                        summary_png_path);


                    fprintf( ...
                        'Summary overview saved:\n%s\n', ...
                        summary_png_path);
                end
            end


            if isgraphics(fig)

                close( ...
                    fig);
            end


        catch ME

            if ~isempty(fig) && ...
                    isgraphics(fig)

                close( ...
                    fig);
            end


            warning( ...
                'save_mean_image_overviews:FigureCreationFailed', ...
                ['Recording %d: unable to create or save the ' ...
                 'mean-image overview figure: %s'], ...
                m, ...
                ME.message);
        end
    end
end


%% ========================================================================
function value = get_metadata_recording_value_mean_overview( ...
    metadata, ...
    field_name, ...
    recording_index)

    value = [];


    if isempty(metadata) || ...
            ~isfield(metadata, field_name)

        return;
    end


    values = ...
        metadata.(field_name);


    if isempty(values)
        return;
    end


    if iscell(values)

        if recording_index > numel(values)
            return;
        end


        value = ...
            values{recording_index};

    else

        if recording_index > numel(values)
            return;
        end


        value = ...
            values(recording_index);
    end
end


%% ========================================================================
function txt = build_plane_coordinate_text_mean_overview( ...
    position_x, ...
    position_y, ...
    position_z, ...
    plane_index)

    x = ...
        get_plane_coordinate_value_mean_overview( ...
            position_x, ...
            plane_index);


    y = ...
        get_plane_coordinate_value_mean_overview( ...
            position_y, ...
            plane_index);


    z = ...
        get_plane_coordinate_value_mean_overview( ...
            position_z, ...
            plane_index);


    parts = {};


    if ~isempty(x) && ...
            isnumeric(x) && ...
            isscalar(x) && ...
            isfinite(x)

        parts{end + 1} = ...
            sprintf( ...
                'X %.1f um', ...
                x);
    end


    if ~isempty(y) && ...
            isnumeric(y) && ...
            isscalar(y) && ...
            isfinite(y)

        parts{end + 1} = ...
            sprintf( ...
                'Y %.1f um', ...
                y);
    end


    if ~isempty(z) && ...
            isnumeric(z) && ...
            isscalar(z) && ...
            isfinite(z)

        parts{end + 1} = ...
            sprintf( ...
                'Z %.1f um', ...
                z);
    end


    if isempty(parts)

        txt = ...
            'coordinates unavailable';

    else

        txt = ...
            strjoin( ...
                parts, ...
                ' | ');
    end
end


%% ========================================================================
function value = get_plane_coordinate_value_mean_overview( ...
    coordinate_data, ...
    plane_index)

    value = [];


    if isempty(coordinate_data)
        return;
    end


    if iscell(coordinate_data)

        if numel(coordinate_data) == 1

            coordinate_data = ...
                coordinate_data{1};

        elseif plane_index <= numel(coordinate_data)

            coordinate_data = ...
                coordinate_data{plane_index};

        else

            return;
        end
    end


    if isstring(coordinate_data) || ...
            ischar(coordinate_data)

        coordinate_data = ...
            str2num( ...
                char(coordinate_data)); %#ok<ST2NM>
    end


    if ~isnumeric(coordinate_data) || ...
            isempty(coordinate_data)

        return;
    end


    if isscalar(coordinate_data)

        value = ...
            double(coordinate_data);

    elseif plane_index <= numel(coordinate_data)

        value = ...
            double( ...
                coordinate_data(plane_index));
    end
end


%% ========================================================================
function images = get_recording_images_mean_overview( ...
    all_images, ...
    recording_index)

    images = {};


    if isempty(all_images)
        return;
    end


    if recording_index > numel(all_images)
        return;
    end


    if isempty( ...
            all_images{recording_index})

        return;
    end


    images = ...
        all_images{recording_index};
end


%% ========================================================================
function img = get_plane_image_mean_overview( ...
    images, ...
    plane_index)

    img = [];


    if isempty(images)
        return;
    end


    if plane_index > numel(images)
        return;
    end


    img = ...
        images{plane_index};


    if isempty(img)

        img = [];
    end
end


%% ========================================================================
function [img1, img2] = match_mean_image_sizes( ...
    img1, ...
    img2)

    if isempty(img1) || ...
            isempty(img2)

        return;
    end


    h = ...
        min( ...
            size(img1, 1), ...
            size(img2, 1));


    w = ...
        min( ...
            size(img1, 2), ...
            size(img2, 2));


    img1 = ...
        img1( ...
            1:h, ...
            1:w);


    img2 = ...
        img2( ...
            1:h, ...
            1:w);
end


%% ========================================================================
function rgb = create_single_channel_rgb( ...
    img, ...
    channel)

    if isempty(img)

        rgb = ...
            zeros( ...
                10, ...
                10, ...
                3);

        return;
    end


    img = ...
        normalize_mean_image_contrast( ...
            img);


    rgb = ...
        zeros( ...
            size(img, 1), ...
            size(img, 2), ...
            3);


    rgb(:, :, channel) = ...
        img;
end


%% ========================================================================
function rgb = create_mean_image_overlay( ...
    gcamp, ...
    electroporated, ...
    electroporated_color_channel)

    [gcamp, electroporated] = ...
        match_mean_image_sizes( ...
            gcamp, ...
            electroporated);


    gcamp = ...
        normalize_mean_image_contrast( ...
            gcamp);


    electroporated = ...
        normalize_mean_image_contrast( ...
            electroporated);


    rgb = ...
        zeros( ...
            size(gcamp, 1), ...
            size(gcamp, 2), ...
            3);


    rgb(:, :, 2) = ...
        gcamp;


    rgb(:, :, electroporated_color_channel) = ...
        electroporated;
end


%% ========================================================================
function img = normalize_mean_image_contrast(img)

    img = ...
        double(img);


    p1 = ...
        calculate_percentile_mean_overview( ...
            img(:), ...
            1);


    p99 = ...
        calculate_percentile_mean_overview( ...
            img(:), ...
            99);


    if ~isfinite(p1)

        p1 = ...
            min(img(:));
    end


    if ~isfinite(p99)

        p99 = ...
            max(img(:));
    end


    if p99 <= p1

        img = ...
            zeros( ...
                size(img));

        return;
    end


    img = ...
        (img - p1) / ...
        (p99 - p1);


    img(img < 0) = ...
        0;


    img(img > 1) = ...
        1;
end


%% ========================================================================
function value = calculate_percentile_mean_overview( ...
    values, ...
    p)

    values = ...
        values( ...
            isfinite(values));


    if isempty(values)

        value = ...
            NaN;

        return;
    end


    values = ...
        sort( ...
            values(:));


    idx = ...
        1 + ...
        (numel(values) - 1) * ...
        p / 100;


    i1 = ...
        floor(idx);


    i2 = ...
        ceil(idx);


    if i1 == i2

        value = ...
            values(i1);

    else

        alpha = ...
            idx - i1;


        value = ...
            (1 - alpha) * values(i1) + ...
            alpha * values(i2);
    end
end


%% ========================================================================
function output_root_folder = get_root_folder_mean_overview( ...
    folders, ...
    recording_index)

    output_root_folder = '';


    if isempty(folders)
        return;
    end


    if recording_index > numel(folders)
        return;
    end


    if isempty( ...
            folders{recording_index})

        return;
    end


    output_root_folder = ...
        folders{recording_index};
end


%% ========================================================================
function age_number = ...
    extract_age_number_mean_overview(age_label)

    age_number = ...
        NaN;


    token = ...
        regexp( ...
            char(string(age_label)), ...
            '[-+]?\d*\.?\d+', ...
            'match', ...
            'once');


    if ~isempty(token)

        age_number = ...
            str2double(token);
    end
end


%% ========================================================================
function show_empty_mean_image_panel( ...
    ax, ...
    message)

    cla(ax);


    axis( ...
        ax, ...
        'off');


    text( ...
        ax, ...
        0.5, ...
        0.5, ...
        message, ...
        'Units', 'normalized', ...
        'HorizontalAlignment', 'center', ...
        'VerticalAlignment', 'middle', ...
        'Color', [0.5 0.5 0.5], ...
        'FontSize', 11);
end


%% ========================================================================
function save_mean_overview_figure( ...
    fig, ...
    file_path)

    try

        exportgraphics( ...
            fig, ...
            file_path, ...
            'Resolution', ...
            300);

    catch

        saveas( ...
            fig, ...
            file_path);
    end
end
