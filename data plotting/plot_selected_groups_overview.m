    ...
    for t = 1:numel(type_names)
        ...
        for g = 1:numel(current_groups)
            ...
            day_table = extract_branch_counts( ...
                day_table, current_type, animal_id, ...
...
function day_table = extract_branch_counts( ...
        day_table, current_type, animal_id, ages, dates, ...
    ...
    for m = 1:n_recordings
        ...
        for p = 1:n_planes
            ...
            new_row = table( ...
                ...
                'VariableNames', { ...
                    'Type', 'Animal', 'Age', 'AgeNumber', 'Date', ...
...
% =========================================================================
function labels = create_animal_labels(types, animals)
    ...
    for i = 1:numel(unique_animals)
        ...
        current_animal = unique_animals(i);

        rows = animals == current_animal;
        associated_types = unique(types(rows));

        % Add the experimental type only when the same animal name
        % appears in several types.
        if numel(associated_types) > 1

            labels(rows) = ...
                types(rows) + " - " + animals(rows);
        end
    end
end
...
function legend_table = initialize_legend_table()
    ...
    legend_table = table( ...
        ...
        string.empty(0, 1), ... % Ages
        'VariableNames', { ...
            'Type', 'Animal', 'DisplayAnimal', 'ColorIndex', ...
            'Red', 'Green', 'Blue', ...
            'AgeMin', 'AgeMax', 'Ages'});
end
...
function fig = plot_selected_groups_layers(day_table, legend_table, current_type)
    ...
    layer_table.DisplayAnimal = create_animal_labels( ...
        layer_table.Type, layer_table.Animal);
