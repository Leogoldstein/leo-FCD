function [chosen_folder_processing_gcamp, chosen_folder_processing_blue] = create_base_folders(date_group_paths, current_gcamp_folders_names_group, current_blue_folders_names_group, daytime, user_choice1, user_choice2, current_animal_group)
    ...
    chosen_folder_processing_gcamp = cell(length(date_group_paths), 1); 
    chosen_folder_processing_blue = cell(length(date_group_paths), 1);
    ...
    for k = 1:length(date_group_paths)
        ...
        elseif ~isempty(user_choice1) && strcmpi(user_choice1, '1')
            if ~isempty(specificSubfolders)
                most_recent_subfolder = specificSubfolders(1);
                chosen_folder_processing_gcamp{k} = fullfile(date_group_paths{k}, most_recent_subfolder.name, current_gcamp_folders_names_group{k});
                disp(['Automatically selected the most recent gcamp subfolder: ', chosen_folder_processing_gcamp{k}]);
                if ~isempty(current_blue_folders_names_group{k})
                    chosen_folder_processing_blue{k} = fullfile(date_group_paths{k}, most_recent_subfolder.name, current_blue_folders_names_group{k});
