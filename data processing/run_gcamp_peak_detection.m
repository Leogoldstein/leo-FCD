function data = run_gcamp_peak_detection( ...
    ...
    for m = 1:numFolders
        ...
        if has_any_detection_group
            [viewer_mode_requested_group, skip_group] = ask_viewer_or_load_only(true, m, 0, 'group');
...
function [viewer_mode, skip_plane] = ask_viewer_or_load_only(has_detection, m, p, cell_type)
    viewer_mode = false;
    skip_plane  = false;
    ...
    if p == 0
        msg = sprintf(['Group %d\nExisting %s detection found.\n\n' ...
                       'What do you want to do?'], m, cell_type);
    else
        msg = sprintf(['Group %d - plane %d\nExisting %s detection found.\n\n' ...
                       'What do you want to do?'], m, p, cell_type);
    end

    answer = questdlg(msg, ...
        'Existing detection', ...
        'viewer mode', 'load only / skip', 'cancel', ...
        'viewer mode');
