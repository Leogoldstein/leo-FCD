function selected_groups = data_checking(selected_groups, include_blue_cells)
    ...
    for t = 1:numel(type_names)
        ...
        for k = 1:numel(selected_groups.(current_type))
            ...
            current_dates_group = ...
                get_dates_from_animal(animal);

            nDates = ...
                get_n_dates_for_choice( ...
                    data, ...
                    checking_choice2);
            ...
            for m = 1:nDates
                ...
                for p = 1:nPlanes
                    ...
                    if isempty(DF_plane)
                        ...
                        fprintf( ...
                            'Date %d plan %d skipped: empty DF\n', ...
                            m, ...
                            p);
                    ...
                    %==================================================
                    % Garder seulement les neurones valides
                    %==================================================
                    valid_neurons_local = ...
                        any(~isnan(DF_plane), 2);
                    ...
                    %==================================================
                    % Nombre de cellules affichées par batch
                    %==================================================
                    ...
                    sampling_rate_plane = ...
                        animal.metadata.gcamp_plane.SamplingRatePlane{m};

                    if isempty(sampling_rate_plane) || ...
                            ~isfinite(sampling_rate_plane) || ...
                            sampling_rate_plane <= 0

                        fprintf( ...
                            'Date %d plan %d skipped: invalid sampling rate', ...
                            m, ...
                            p);
                    ...
                    % Motion energy
                    %
                    % On garde tous les points de motion energy.
                    % L'axe temporel utilise fs_motion.
                    %==================================================
                    motion_energy = ...
                        get_motion_energy( ...
                            data, ...
                    ...
                    batch_slider = uicontrol( ...
                        ...
                        'SliderStep', ...
                        [ ...
                            1/max(num_batches-1,1), ...
                            1/max(num_batches-1,1) ...
                        ], ...
                        'Units', 'normalized', ...
                        'Position', ...
                        [0.08 0.235 0.88 0.025]);
                    ...
                    frame_slider = uicontrol( ...
                        ...
                        'SliderStep', ...
                        [ ...
                            1/max(num_columns-1,1), ...
                            100/max(num_columns-1,1) ...
                        ], ...
                        'Units', 'normalized', ...
                        'Position', ...
                        [0.08 0.205 0.88 0.025]);
                    ...
                    setappdata( ...
                        main_fig, ...
                        'focus_pre_margin_frames', ...
                        100);

                    setappdata( ...
                        main_fig, ...
                        'focus_post_margin_frames', ...
                        100);

                    setappdata( ...
                        main_fig, ...
                        'focus_play_pause_sec', ...
                        0.35);
                    ...
                    setappdata( ...
                        main_fig, ...
                        'selected_cells_movie', ...
                        []);

                    setappdata( ...
                        main_fig, ...
                        'movie_outline_handles', ...
                        []);
                    ...
                    setappdata( ...
                        main_fig, ...
                        'frame_slider_listener', ...
                        frame_listener);

                    try

                        frame_slider.ValueChangedFcn = ...
                            @(src, ~) ...
                            update_frame_cursor_and_movie( ...
                                main_fig, ...
                                round(get(src, 'Value')));
...
function movie_data = open_movie_for_animal(animal, m, p)
    ...
    current_suite2p_group = ...
        animal.paths.suite2p;

    if size(current_suite2p_group,1) < m || ...
            isempty(current_suite2p_group{m,1})

        fprintf( ...
            'No suite2p path for date %d.\n', ...
            m);
    ...
    planes_cell = ...
        current_suite2p_group{m,1};

    if ~iscell(planes_cell) || ...
            numel(planes_cell) < p || ...
            isempty(planes_cell{p})

        fprintf( ...
            'No suite2p path for date %d plane %d.\n', ...
            m, ...
            p);
    ...
    current_suite2p_path = ...
        char(string(current_suite2p_path));
...
function movie_data = open_reg_tif_stack_viewer(reg_tif_dir, p)
    ...
    infos = ...
        cell(numel(fullPaths),1);

    pagesPerFile = ...
        zeros(numel(fullPaths),1);
    ...
    fprintf( ...
        'Loaded %d frames from %d tif files.\n', ...
        nFrames, ...
        numel(fullPaths));
end
...
function update_movie_stack(main_fig, frame_index)
    ...
    if ~isfield(movie_data, 'movie_fig') || ...
            isempty(movie_data.movie_fig) || ...
            ~isvalid(movie_data.movie_fig)
        return;
    end

    if ~isfield(movie_data, 'hImg') || ...
            isempty(movie_data.hImg) || ...
            ~isvalid(movie_data.hImg)
        return;
    end
    ...
    if fileIdx == 1
        ...
        pageIdx = ...
            frame_index - ...
            movie_data.cumulativePages(fileIdx - 1);
    end
    ...
    try
        ...
        if isfield(movie_data, 'ax') && ...
                isvalid(movie_data.ax)

            title( ...
                movie_data.ax, ...
                sprintf( ...
                    'Frame %d / %d | file %d page %d', ...
                    frame_index, ...
                    movie_data.nFrames, ...
                    fileIdx, ...
                    pageIdx));
        ...
        warning( ...
            'Could not read movie frame %d: %s', ...
            frame_index, ...
            ME.message);
    end
end
...
function update_movie_outlines(main_fig)
    ...
    for i = 1:numel(neurons_in_batch)
        ...
        if local_idx > ...
                numel(valid_neuron_indices)
            continue;
        end

        original_idx = ...
            valid_neuron_indices(local_idx);
    ...
    switch checking_choice2
        ...
        case '3'
            ...
            if ~isempty(valid_combined) && ...
                ...
                outline_gcampx_plane = ...
                    outline_all_x(valid_combined);

                outline_gcampy_plane = ...
                    outline_all_y(valid_combined);
...
function value = safe_get_plane(branch, fieldname, m, p)
    ...
    if isempty(branch) || ...
            ~isfield(branch, fieldname) || ...
            isempty(branch.(fieldname))
        return;
    end
...
function isort1_plane = prepare_isort1( ...
        isort1_raw, ...
        valid_local_indices, ...
        nCellsOriginal)
    ...
    if isempty(isort1_plane)
        ...
        isort1_plane = ...
            (1:numel(valid_local_indices)).';
    end
end


function update_batch_display( ...
        batch_slider, ...
        DF, ...
        isort1, ...
        batch_size, ...
...
function update_raster_subplot( ...
        ax1, ...
        DF, ...
        isort1, ...
        neurons_in_batch, ...
    ...
    if isempty(t_df)
        rect_width = 1 / sampling_rate_plane;
    else
        rect_width = max(t_df(end), 1 / sampling_rate_plane);
    end
...
function update_traces_subplot( ...
        ax2, ...
        DF, ...
        neurons_in_batch, ...
        num_columns, ...
...
function update_frame_cursor_and_movie( ...
        fig_handle, ...
        frame_idx)
    ...
    if ~isempty(frame_slider) && ...
            isvalid(frame_slider)
    ...
    % The movie itself remains indexed in plane frames.
    update_movie_stack( ...
        fig_handle, ...
        frame_idx);
end
...
function play_focus_change(main_fig, mode)
    ...
    if isempty(pre_margin)
        pre_margin = 10;
    end

    if isempty(post_margin)
        post_margin = 10;
    end
    ...
    switch lower(mode)
        ...
        case 'replay'
            ...
            if isempty(idx) || ...
                    ~isfinite(idx) || ...
                    idx < 1 || ...
                    idx > size(focus_segs,1)

                fprintf( ...
                    'No focus segment selected.\n');
...
function focus_segs = get_motion_focus_segs(data, m)
    ...
    if size(focus_segs, 2) ~= 2
        ...
        warning( ...
            'focus_segs_group{%d} should be Nx2 [start end].', ...
            m);
...
function motion_energy = get_motion_energy(data, m)
    ...
    if ~isfield( ...
            ...
            'motion_energy_group') || ...
            isempty( ...
                data.motion.motion_energy_group)
...
function nDates = get_n_dates_for_choice( ...
        data, ...
        checking_choice2)
...
function nPlanes = get_n_planes_for_choice( ...
        data, ...
        checking_choice2, ...
        m)
...
function dates = get_dates_from_animal(animal)
    ...
    if isfield(animal, 'metadata') && ...
            ~isempty(animal.metadata)
...
function label = get_label_from_cell( ...
        c, ...
        idx, ...
        default_label)
