function build_rasterplots(all_DF, all_isort1, all_MAct, raster_path, current_animal_group, current_dates_group, current_age_group)
    ...
    % - raster_path: Path for saving the raster plot figure
    ...
    fig_save_path = fullfile(raster_path, sprintf('%s_rastermap.png', strrep(current_animal_group, ' ', '_')));
    ...
    if ~exist(fig_save_path, 'file')  % Check if the figure already exists
        ...
        global_ymax = ceil(global_ymax * 10) / 10;
        ...
        for k = 1:length(current_dates_group)  % Iterate over the dates for the current group
            try
                ...
                % Extract data from the input cell arrays for the current group and date
                DF = all_DF{idx};
