...
%   v1_26_07_22
    ...
    if ~isempty(gcamp_output_folder)
        ...
        if ~exist(gcamp_output_folder, 'dir')
            mkdir(gcamp_output_folder);
        end

        local_png_path = fullfile( ...
            gcamp_output_folder, ...
            'random_peak_preview.png');
    end
    ...
    if ~isempty(gcamp_output_folder)
        ...
        cleaned_gcamp_folder = remove_trailing_filesep( ...
            gcamp_output_folder);

        [~, version_name] = fileparts(cleaned_gcamp_folder);
    end

    version_name = sanitize_filename_part(version_name);

    % =========================================================
    % Extraction de l'animal et de la date
    %
    % Chemin attendu :
    % type\line\animal\date\TSeries\...\version
    % =========================================================
    animal_name = '';
    date_name   = '';
    ...
    if ~isempty(gcamp_output_folder)
        ...
        [animal_name, date_name] = ...
            extract_animal_and_date_from_path( ...
                gcamp_output_folder);
    end
    ...
    if isempty(date_name)
        date_name = 'unknown_date';
    end

    animal_name = sanitize_filename_part(animal_name);
    date_name   = sanitize_filename_part(date_name);
    ...
    figure_base_name = sanitize_filename_part(figure_base_name);
...
function clean_part = sanitize_filename_part(value)
%SANITIZE_FILENAME_PART
%
% Nettoie une valeur destinée à être utilisée dans un nom de fichier.
%
% Remplace notamment :
%   espaces
...
% par des underscores.
    ...
    % Remplacer les espaces et tabulations par des underscores
    value = regexprep( ...
        value, ...
        '\s+', ...
        '_');

    % Remplacer plusieurs underscores consécutifs
    value = regexprep( ...
        value, ...
        '_+', ...
        '_');

    % Retirer les points et underscores en début ou fin
    value = regexprep( ...
        value, ...
        '^[._]+|[._]+$', ...
        '');
