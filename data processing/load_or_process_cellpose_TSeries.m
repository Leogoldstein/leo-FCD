        ...
        if ~isempty(cellpose_files_canal)
            if numel(cellpose_files_canal) > 1
                ...
                if isequal(selected_file, 0)
                    disp('Aucun fichier sélectionné. Opération annulée par l''utilisateur.');
                end
            ...
            if isfile(aligned_image_path)
                ...
                currentFolder = folders_groups{1}{:, 1};
                ...
                canal_str = str2double(erase(canal_str, 'Ch'));
                meanImg_channels{canal_str} = image_tiff; 
