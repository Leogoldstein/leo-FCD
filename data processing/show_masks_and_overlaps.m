function [matched_cellpose_idx, matched_gcamp_idx] = show_masks_and_overlaps(all_gcamp_props, all_props_cellpose, all_outlines_x_cellpose, all_outlines_y_cellpose, all_outline_gcampx, all_outline_gcampy, sorted_date_group_paths, all_meanImg, aligned_images)
    ...
    for m = 1:length(sorted_date_group_paths)
        try
            ...
            for ncell = 1:length(all_outlines_x_cellpose{m})
                plot(all_outlines_x_cellpose{m}{ncell}, all_outlines_y_cellpose{m}{ncell}, '.', 'MarkerSize', 1, 'Color', 'b');
            end
